...
% split into sections
ecg_uncon = data(138400:285400);figure;plot(time(138400:285400),ecg_uncon);xlabel('time (s)');
ecg_con15 = data(285401:416900);figure;plot(time(285401:416900),ecg_con15);
ecg_con50 = data(495800:640900);figure;plot(time(495800:640900),ecg_con50);
...
figure('Position',[250 250 800 200]);stem(k,pacf_uncon,'DisplayName','Original');hold on;stem(k,pacf_uncon_norm,'DisplayName','Zero-mean')
...
figure('Position',[250 250 800 200]);stem(k,pacf_con15,'DisplayName','Original');hold on;stem(k,pacf_con15_norm,'DisplayName','Zero-mean')
...
E=zeros(N,p);
o = ones(1,p);
E(1,:) = (x(1,:)-o*signal(1)).^2;

for i=2:N 
    E(i,:) = (x(i,:)-o*signal(i)).^2 + E(i-1,:);
end
...
%% Chapter 3 - Spectral estimation and modelling
...
figure('Position',[250 250 800 400]);plot(-0.5:1/length(pgm_uncon):0.5-1/length(pgm_uncon),pgm_uncon,'LineWidth',0.8);title('Periodogram of RRI for unconstrained breathing');xlabel('Normalised frequency f');ylabel('Periodogram P(f)');grid on;xlim([-0.3,0.3])
figure('Position',[250 250 800 400]);plot(-0.5:1/length(pgm_con15):0.5-1/length(pgm_con15),pgm_con15,'LineWidth',0.8);title('Periodogram of RRI for breathing constrained at 15 bpm');xlabel('Normalised frequency f');ylabel('Periodogram P(f)');grid on;xlim([-0.3,0.3])
figure('Position',[250 250 800 400]);plot(-0.5:1/length(pgm_con50):0.5-1/length(pgm_con50),pgm_con50,'LineWidth',0.8);title('Periodogram of RRI for breathing constrained at 50 bpm');xlabel('Normalised frequency f');ylabel('Periodogram P(f)');grid on;xlim([-0.3,0.3])
...
figure('Position',[250 250 800 400]);plot(-0.5:1/length(avg_pgm):0.5-1/length(avg_pgm),avg_pgm,'LineWidth',0.8);title('Average periodogram of RRI for breathing constrained at 50bpm with window size 50');xlabel('Normalised frequency f');ylabel('Periodogram P(f)');grid on;xlim([-0.3,0.3])


% periodogram function
function [p_out] = pgmest(x)
    N = length(x);
    p_out = (1/N)*abs(fftshift(fft(x))).^2; 
end
